    ...
    methods:{
        signup(){
            if(this.Isuniq==-1) alert("중복을 확인하지 않았습니다. 확인하세요")
                ...
                this.$http.post('http://10.41.83.72:8888/auth/co/signup', {user: this.user}).then((response) => {
        ...
        dupcheck(){
            this.$http.get('http://10.41.83.72:8888/auth/co/dupcheck', {params:{id : this.user.id}}).then((res)=>{

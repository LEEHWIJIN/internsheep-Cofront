...
<script>
  ...
  export default{
      ...
      async created(){
        ...
        await this.$http.get('http://106.10.46.121:8888/admin/recentApplyTerm').then((response) => {
          this.applyTerm = {
      ...
      methods: {
        applyStd(cName){
            this.$http.get('http://106.10.46.121:8888/std/mypage/applyStatus',{params:{sLoginID : this.user.loginId}}).then((response)=>{
            if(response.data != '0'){
                alert("이미 지원을 한 상태 입니다.")
            }
            else{
                this.$http.post('http://106.10.46.121:8888/std/mypage/applyCo',{cName : cName, sLoginID : this.user.loginId,applySemester: this.applyTerm.applySemester,applyOrder:this.applyTerm.applyOrder}).then((response) => {
                    if(response.data == '0'){
